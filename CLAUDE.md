...
- `models/stationary/vae.py` — `VAE` (encodeur + décodeur), plus `IndirectDecoder` qui projette θ dans l'espace latent du VAE et réutilise son décodeur gelé.
- `models/stationary/direct_decoder.py` — `DirectDecoder` et `DirectDecoderDenseOut` (θ → U directement, sans AE).
- `models/stationary/ae_svd.py` — `AutoencoderSVD`, `IndirectDecoderSVD`, `compute_fixed_svd_basis`.
...
**`models/transient/laplace_surrogate.py`** :
...
**`models/transient/svd_surrogate.py`** :
