...
\begin{abstract}
This document derives rigorously the bias--variance decomposition of the total reconstruction error for a Laplace-domain neural surrogate model combined with a regularised inversion step. Using this theoretical framework, we formulate an optimization problem to find the optimal integration contour frequencies that minimize the expected error. We also discuss approximating the surrogate error using the mean Singular Value Decomposition (SVD) residual.
\end{abstract}
...
\begin{itemize}
    ...
    \item The superscript asterisk ($^*$) designates the optimal solution to an equation or an optimization problem (e.g., the reconstructed field $\mathbf{v}^*$).
...
We decompose the total reconstruction error $\mathbf{v}(\theta) - \mathbf{v}^*(\theta)$ by introducing the \emph{exact} Laplace coefficients $\mathbf{u}(\theta) = \mathbf{F}_{st}\mathbf{v}(\theta)$. Adding and subtracting $\mathbf{A}^{-1}\mathbf{F}_{st}^H\mathbf{u}(\theta)$:

\begin{align}
  \mathbf{v}(\theta) - \mathbf{v}^*(\theta)
  &= \mathbf{v}(\theta)
     - \mathbf{A}^{-1}\mathbf{F}_{st}^H\mathbf{F}_{st}\mathbf{v}(\theta)
...
\begin{equation}
  \mathbf{v} - \mathbf{A}^{-1}\mathbf{F}_{st}^H\mathbf{F}_{st}\mathbf{v}
  \;=\; \mathbf{A}^{-1}\bigl(\mathbf{A} - \mathbf{F}_{st}^H\mathbf{F}_{st}\bigr)\mathbf{v}
  \;=\; \mathbf{A}^{-1}\mathbf{L}_{\mathrm{reg}}\mathbf{v}.
\end{equation}
...
\begin{theorem}[Bias--variance bound]
  ...
  \begin{equation}
    ...
    \;\leq\;
    \underbrace{
      \norm{\mathbf{A}^{-1}\mathbf{L}_{\mathrm{reg}}\mathbf{v}(\theta)}
...
Therefore, we aim to jointly optimize $\mathbf{s}$, $\lambda$, and $\alpha$ to minimize the expected mean reconstruction error $\E_\theta\!\left[\norm{\mathbf{v}(\theta) - \mathbf{v}^*(\theta)}\right]$. Since evaluating this true error directly is challenging without ground truth for every instance, we treat these hyperparameters as learnable parameters and optimize a surrogate loss function $\mathcal{L}(\mathbf{s}, \lambda, \alpha)$. This loss is derived directly from the bias--variance upper bound :
\begin{align}
    \E_\theta\!\left[\norm{\mathbf{v}(\theta) - \mathbf{v}^*(\theta)}\right] \;&\leq\; \mathcal{L}(\mathbf{s}, \lambda, \alpha), \\
...
where
\begin{align}
    \mathcal{L}_{\mathrm{bias}}(\mathbf{s}, \lambda, \alpha) &\;=\; \E_\theta\!\left[\norm{\mathbf{A}(\mathbf{s}, \lambda, \alpha)^{-1}\mathbf{L}_{\mathrm{reg}}(\lambda, \alpha)\mathbf{v}(\theta)}\right], \\
...
\begin{equation}
    \E_{\theta \in \Theta}\!\left[\norm{\mathbf{u}(s_i, \theta) - \hat{\mathbf{u}}_{\mathrm{lin}}(s_i, \theta)}^2\right] \;=\; \frac{1}{|\Theta|} \sum_{m=1}^{|\Theta|} \norm{\mathbf{u}(s_i, \theta_m) - \hat{\mathbf{u}}_{\mathrm{lin}}(s_i, \theta_m)}_{\C^{N_x}}^2 \;=\; \frac{1}{|\Theta|} \norm{\mathbf{U}^{(s_i)} - \hat{\mathbf{U}}_{\mathrm{lin}}^{(s_i)}}_F^2
\end{equation}
...
\subsection{Conclusion: The Final Proxy Objective}

We can finally derive the final explicit proxy objective function used in our optimization routine:
\begin{equation}
    \boxed{ \mathcal{L}^{\mathrm{proxy}}(\mathbf{s}, \lambda, \alpha) \;=\; \frac{1}{|\Theta|} \sum_{\theta \in \Theta} \norm{\mathbf{A}^{-1}\mathbf{L}_{\mathrm{reg}}\mathbf{v}(\theta)} + \frac{1}{\lambda_{\mathrm{AE}}} \norm{\mathbf{A}^{-1}\mathbf{F}_{st}^H} \cdot \sqrt{ \frac{1}{|\Theta|} \sum_{s_i \in \mathbf{s}} \sum_{j > r} \bigl(\sigma_j^{(s_i)}\bigr)^2 } }
\end{equation}
Where:
\begin{itemize}
    \item $\mathbf{v}(\theta) \in \R^{N_x N_t}$ is the exact transient physical field for the simulation instance $\theta \in \Theta$.
    \item $\mathbf{F}_{st}(\mathbf{s})$ is the discrete spatio-temporal Laplace transform operator associated with the chosen contour frequencies $\mathbf{s}$.
    \item $\mathbf{L}_{\mathrm{reg}}(\lambda, \alpha)$ is the regularisation penalty matrix, controlling amplitude ($\lambda$) and spatio-temporal smoothness ($\alpha_x, \alpha_t$).
    \item $\mathbf{A} = \mathbf{F}_{st}^H\mathbf{F}_{st} + \mathbf{L}_{\mathrm{reg}}$ is the invertible system matrix of the normal equations. Thus, $\mathbf{A}^{-1}\mathbf{F}_{st}^H$ acts as the actual regularised Inverse Laplace Transform operator.
    \item $\sigma_j^{(s_i)}$ are the singular values of the exact Laplace coefficients dataset matrix at frequency $s_i$. The square root term computes the global root-mean-square residual of the optimal linear SVD compression beyond the Autoencoder's bottleneck latent dimension $r$, serving as a worst-case upper bound proxy for the surrogate's prediction error.
    \item $\lambda_{\mathrm{AE}} \geq 1$ is an empirical hyperparameter scaling down the SVD variance to account for the superior expressivity of the non-linear Autoencoder.
\end{itemize}
